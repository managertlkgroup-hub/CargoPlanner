...
export interface PackSettings {
  ...
  /** Общий включатель зазоров — если false, все зазоры игнорируются */
  gapsEnabled: boolean;
  /** Зазор между грузами и стенками кузова (мм). 0 — без зазора */
  gap: number;
  /** Зазор от стенок кузова по периметру (мм). 0 — без зазора */
  gapWalls: number;
  /** Зазор между рядами грузов по ширине кузова, ось Z (мм). 0 — без зазора */
  gapWidth: number;
  /** Зазор между рядами грузов по длине кузова, ось X (мм). 0 — без зазора */
  gapLength: number;
}
...
/** Глобальные единицы измерения (display/input) */
