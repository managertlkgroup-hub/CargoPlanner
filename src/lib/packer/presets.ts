...
export interface CargoPreset {
  ...
  nameKey?: string;
  /** Идентификатор исходного стандартного пресета, из которого был «продвинут» этот
   *  пользовательский пресет при редактировании. Не влияет на отображение — служит
   *  для того, чтобы повторное редактирование не создавало дубликаты. */
  sourceId?: string;
  length: number;
  width: number;
  height: number;
